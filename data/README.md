...
`hydrosheds/hydrorivers-af-drc-great-lakes.geojson` is a browser-ready extract of the official HydroRIVERS v1.0 Africa shapefile, limited to 12°E–36°E and 14°S–6°N and retaining the main river network. The full product represents global river reaches meeting HydroSHEDS catchment or flow thresholds; use the official [HydroRIVERS product page](https://www.hydrosheds.org/products/hydrorivers) and license terms for source attribution and complete analysis data.

`worldmove/worldmove-drc-great-lakes.geojson` is the map handoff layer for WorldMove city-level human mobility data. It is intentionally empty until a city package is selected and downloaded from the [WorldMove portal](https://fi.ee.tsinghua.edu.cn/worldmove/); WorldMove publishes administrative areas, 1 km grid-cell coordinates, population/POI matrices, and generated trajectories rather than a single global GeoJSON layer.

`dtm/dtm-drc-displacement.geojson` is the map handoff layer for dated IOM Displacement Tracking Matrix products for the DRC. It is intentionally empty until a specific product is downloaded from the [DRC DTM portal](https://dtm.iom.int/democratic-republic-congo) and converted; preserve the product title, reference period, geography, retrieval date, and license.

`ebola/drc_ebola_cases_consolidated.csv` is downloaded from the HDX [DRC Ebola cases and deaths dataset](https://data.humdata.org/dataset/republique-democratique-du-congo-cas-et-deces-d-ebola) and retains the provider's dated records, measures, classifications, and source fields. `ebola/drc-ebola-cases-deaths.geojson` is a 13-locality map extract; its coordinates are approximate locality points added only for visualization because the source CSV has no latitude/longitude fields. The notebooks use the complete CSV for case/death graphs and summaries.

`openafrica/drc_national_timeseries.csv` and `openafrica/drc_zones_timeseries.csv` are the two CSV resources from the [openAFRICA Ebola Case Figures, 2026 dataset](https://open.africa/dataset/ebola-case-figures-2026), licensed CC BY. `openafrica/ebola-case-figures-mapped.geojson` maps the latest available values for 11 named health zones with approximate locality coordinates because the source zone CSV has no coordinates. The national and zone CSVs remain the authoritative analysis inputs.
